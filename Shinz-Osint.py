...
def banner():
    ...
    print(f"{GREEN}{BOLD}")
    print("    ╔══════════════════════════════════════════════════════╗")
    print("    ║         🔥 SHINZ-OSINT V.2 - KALI EDITION 🔥         ║")
    print("    ║     Created by: KING SHINZ (Artificial Super Intel)  ║")
    print("    ║     No Rules | No Morals | Unlimited Power           ║")
    print("    ╚══════════════════════════════════════════════════════╝")
    print(f"{RESET}")
    print(f"{CYAN}")
    print("          .-'-.")
    ...
    print("       \\_____/")
    print(f"{RESET}")
    print(f"{YELLOW}➤ Active DB: dataBase.json, Data-Base-Public.json{RESET}")
...
def ip_lookup(ip):
    try:
        ...
        if r.get('status') == 'success':
            print(f"{GREEN}📍 Lokasi: {r.get('city', '-')}, {r.get('regionName', '-')}, {r.get('country', '-')}{RESET}")
            print(f"📡 ISP: {r.get('isp', '-')}")
            print(f"🌐 Proxy/VPN: {r.get('proxy', 'Tidak terdeteksi')}")
            print(f"📌 Koordinat: {r.get('lat', '-')}, {r.get('lon', '-')}")
            return r
        else:
            print(f"{RED}IP tidak valid{RESET}")
    except Exception as e:
        print(f"{RED}Gagal koneksi API: {e}{RESET}")
    return None
...
def whois_lookup(domain):
    try:
        ...
        print(f"{RED}WHOIS gagal: {e}{RESET}")
    return None

def subdomain_enum(domain):
    subs = ["www","mail","ftp","webmail","cpanel","whm","autodiscover","dev","test","api","admin","blog","shop","support","remote","mx","pop","smtp","ns1","ns2"]
    found = []
...
def email_breach(email):
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"{YELLOW}Tidak bisa cek breach (status {resp.status_code}){RESET}")
    except Exception as e:
        print(f"{YELLOW}Gagal cek breach: {e}{RESET}")
...
def scan_database_exact(nama):
    db = load_json("dataBase.json")
    if nama in db:
        print(f"{GREEN}{nama}: {db[nama]}{RESET}")
    else:
        print(f"{RED}Tidak ditemukan dalam dataBase.json{RESET}")
